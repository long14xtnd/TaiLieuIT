	...
	private void computeProbabilities(String filename, Map<String, Double> prob) {
		...
		try {
			...
			while (in.hasNextLine()) {
				...
				/*
				 * for (int i = 0; i < words.length; i++) { for (String key : prob.keySet()) {
				 * if(key.contains(words[i])){ if (key.contains("_<=50K"))
				 * System.out.println(words[i]+" yolo "+key); prob_LE50K =
				 * prob_LE50K*prob.get(key);
				 * 
				 * }else if(key.contains(words[i])){ if(key.contains("_>50K")) prob_GT50K =
				 * prob_GT50K*prob.get(key); } } }
				 */
				...
				prob_LE50K = pr_LE50K * sec_prob_le50k * third_pr_LE50K * forth_prob_le50k * fifth_pr_LE50K
						* sixth_prob_le50k * seventh_prob_le50k * eigth_prob_le50k * ninth_prob_le50k * tenth_prob_le50k
						* eleven_pr_LE50K * twelve_pr_LE50K * thirteen_pr_LE50K * fourteen_prob_le50k * 0.759;
				prob_GT50K = pr_GT50K * sec_prob_gt50k * third_pr_GT50K * forth_prob_gt50k * fifth_pr_GT50K
						* sixth_prob_gt50k * seventh_prob_gt50k * eigth_prob_gt50k * ninth_prob_gt50k * tenth_prob_gt50k
						* eleven_pr_GT50K * twelve_pr_GT50K * thirteen_pr_GT50K * fourteen_prob_gt50k * 0.241;
